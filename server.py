...
#         evaluate_metrics_aggregation_fn=weighted_average, 
#         #---Chnages Sinchan
#         on_fit_config_fn=lambda rnd: {"round": rnd},   # send round info during training
#         on_evaluate_config_fn=lambda rnd: {"round": rnd}  # send round info during evaluation
#     )
...
# --- 6. START THE MLFLOW EXPERIMENT RUN ---
with mlflow.start_run(run_name="FedXViT_Local_Sim_with_XAI") as run:
    run_id = run.info.run_id
    print(f"🔥 Starting MLflow Run: {run_id}")
    print(f"Follow progress in Azure ML Studio: {mlflow.get_tracking_uri()}")
    ...
    if simulation_success:
        ...
        # --- 9. GENERATE AND UPLOAD XAI VISUALIZATIONS ---
        print("\n--- [SERVER LOG] Generating XAI visualizations for model interpretability... ---")
        
        # Run the xai.py script as a separate process
        try:
            subprocess.run(["python", "xai.py"], check=True)
            
            # Log the entire output folder to Azure ML artifacts
            output_dir = "xai_outputs"
            if os.path.exists(output_dir):
                mlflow.log_artifacts(output_dir, artifact_path="xai_results")
                print(f"✅ XAI outputs from '{output_dir}' successfully uploaded to Azure ML Run.")
            else:
                print(f"⚠️ XAI output directory '{output_dir}' not found. Skipping artifact upload.")
                
        except subprocess.CalledProcessError as e:
            print(f"🚨 FAILED to run xai.py script. Error: {e}")
        except Exception as e:
            print(f"🚨 An unexpected error occurred during XAI processing: {e}")

        print(f"\n✅ Simulation finished. All results logged to Azure ML Run ID: {run_id}")
